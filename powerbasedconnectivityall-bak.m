function powerbasedconnectivityall(eegpatientl, eegconditionl, centerfrequenciesl)
...
%eegcondition = 'EC_POST'; eegpatientl = { 'TWH027','TWH024','TWH028','TWH030', 'TWH031','TWH033','TWH034'};
...
for indpat=1:length(eegpatientl)
    ...
    for indcond=1:length(eegconditionl)
        ...
        for indexfreq = 1:length(centerfrequenciesl) % delta, theta, alpha, beta, gamma
            ...
            powerbasedconnectivityperpatient(eegpatient, eegcondition, centerfreq);
        end
    end
end
end

function [ corr_matrix ] = powerbasedconnectivityperpatient(eegpatient, eegcondition, centerfreq)
